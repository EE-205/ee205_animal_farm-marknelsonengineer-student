...
def print_shields_io_tags():
    ...
    brand = f"style={STYLE}"
    repo1 = f"![X]({url}/languages/code-size/{GITHUB_USERNAME}/{GITHUB_REPO}?{brand})"
    repo2 = f"![X]({url}/repo-size/{GITHUB_USERNAME}/{GITHUB_REPO}?{brand})"
    repo3 = f"![X]({url}/contributors/{GITHUB_USERNAME}/{GITHUB_REPO}?{brand})"

    commit1 = f"![X]({url}/commit-activity/w/{GITHUB_USERNAME}/{GITHUB_REPO}?{brand})"
    commit2 = f"![X]({url}/last-commit/{GITHUB_USERNAME}/{GITHUB_REPO}?{brand})"
    issue1 = f"![X]({url}/issues-raw/{GITHUB_USERNAME}/{GITHUB_REPO}?{brand})"
    issue2 = f"![X]({url}/issues-closed-raw/{GITHUB_USERNAME}/{GITHUB_REPO}?{brand})"
    social1 = f"![X]({url}/forks/{GITHUB_USERNAME}/{GITHUB_REPO}?{brand})"
    social2 = f"![X]({url}/stars/{GITHUB_USERNAME}/{GITHUB_REPO}?{brand})"
    ...
    print(
        "|--------------------------------------|---------------------------|-------------------------|---------------------------|")
    print(
        f"| {repo1} <br/> {repo2} <br/> {repo3} | {commit1} <br/> {commit2} | {issue1} <br/> {issue2} | {social1} <br/> {social2} |")
...
def print_tags():
    ...
    if len(tags) == 0:
        ...
        result = subprocess.run(
            ['git', 'for-each-ref', '--sort=creatordate', '--format',
             '|%(refname:strip=2)|%(creatordate)|%(authorname)|',
             'refs/tags'], stdout = subprocess.PIPE)
...
def process_script_files(row_name, file_list):
    ...
    print(
        f"|{row_name}|{num_files:n}|{total_lines:n}|{total_bytes:n}|{total_working_lines:n}|{total_working_bytes:n}|")
